...
class TriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create try" do
    assert_difference('Try.count') do
      post tries_url, params: { try: { frame_id: @try.frame_id, name: @try.name, score: @try.score } }
    end
  ...
  test "should update try" do
    patch try_url(@try), params: { try: { frame_id: @try.frame_id, name: @try.name, score: @try.score } }
